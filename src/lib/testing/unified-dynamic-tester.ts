...
export class UnifiedDynamicTester {
   ...
   *
   * Only removes fields if schema metadata (Zod, OpenAPI, or Prisma-style description) indicates auto-generation.
   * No longer uses pattern-based fallback.
   */
      ...
      if (schema) {
        ...
        if (unwrapped instanceof z.ZodObject) {
          ...
          for (const fieldName of Object.keys(cleanedData)) {
            const fieldSchema = shape[fieldName];
            if (this.shouldRemoveFieldForCreation(fieldName, schema, fieldSchema)) {
   ...
   *
   * Uses only schema metadata for detection. No pattern fallback.
   */
  private shouldRemoveFieldForCreation(
    ...
    if (unwrapped instanceof z.ZodObject) {
      const shape = unwrapped.shape;
      const fieldSchemaObj = fieldSchema || shape[fieldName];

      if (fieldSchemaObj) {
        // Use the same logic as shouldSkipFieldForDefaults but specifically for creation
        return this.isAutoGeneratedField(fieldName, fieldSchemaObj);
      }
    }

    // Fallback to pattern-based detection
    return this.isAutoGeneratedFieldPattern(fieldName, fieldSchema);
  }

  /**
   * Dynamically check if a field is auto-generated using schema metadata, OpenAPI, or Zod only.
   *
   * Returns true only if explicit metadata indicates auto-generation (e.g., readOnly, auto-generated, autoincrement, uuid, @id, @default(autoincrement()), @default(uuid())).
   * If no metadata is available, returns false (does not guess or use pattern fallback).
   */
  private isAutoGeneratedFieldPattern(_fieldName: string, fieldSchema?: any): boolean {
    // 1. Check schema metadata (Zod or OpenAPI)
    if (fieldSchema) {
      // OpenAPI/JSON Schema: readOnly or description
      if (fieldSchema.readOnly === true) return true;
      if (typeof fieldSchema.description === "string") {
        const desc = fieldSchema.description.toLowerCase();
        if (
          desc.includes("auto-generated") ||
          desc.includes("read-only") ||
          desc.includes("autoincrement") ||
          desc.includes("uuid")
        ) {
          return true;
        }
        // Prisma/DB: look for @id, @default(autoincrement()), @default(uuid()) in description
        if (
          desc.includes("@id") ||
          desc.includes("@default(autoincrement())") ||
          desc.includes("@default(uuid())")
        ) {
          return true;
        }
      }
      // Zod: _def.description
      if (fieldSchema._def?.description) {
        const desc = fieldSchema._def.description.toLowerCase();
        if (
          desc.includes("auto-generated") ||
          desc.includes("read-only") ||
          desc.includes("autoincrement") ||
  ...
  /**
   * Check if a field has auto-generated characteristics (strictly metadata-driven)
   *
   * Returns true only if explicit schema metadata (Zod, OpenAPI, or Prisma annotation in description) indicates auto-generation.
   * Does NOT use any field name patterns or guesses.
   */
  private isAutoGeneratedField(fieldName: string, fieldSchema: z.ZodTypeAny): boolean {
    // Check Zod schema description for auto-generation hints
    if (fieldSchema._def?.description) {
      const desc = fieldSchema._def.description.toLowerCase();
      if (
        desc.includes("auto-generated") ||
    ...
    // Check OpenAPI/JSON Schema: readOnly or description
    if ((fieldSchema as any).readOnly === true) return true;
    if (typeof (fieldSchema as any).description === "string") {
      const desc = (fieldSchema as any).description.toLowerCase();
      if (
        desc.includes("auto-generated") ||
        desc.includes("read-only") ||
        desc.includes("autoincrement") ||
    ...
    // Check OpenAPI spec for field descriptions (for HeyAPI-generated schemas)
    const openAPIDescription = this.getOpenAPIFieldDescription(fieldName, fieldSchema);
    if (openAPIDescription) {
      const desc = openAPIDescription.toLowerCase();
      if (
        desc.includes("auto-generated") ||
        desc.includes("read-only") ||
        desc.includes("autoincrement") ||
    ...
    // If no explicit metadata, do not guess
    return false;
  }
  ...
  /**
   * Get field description from OpenAPI spec for HeyAPI-generated schemas
   */
  private getOpenAPIFieldDescription(
    fieldName: string,
    _fieldSchema: z.ZodTypeAny
  ): string | null {
    // Try to find the schema in OpenAPI spec by matching field characteristics
    if (!this.openAPISpec.components?.schemas) {
      return null;
    }

    // Look for schemas that contain this field
    for (const [_schemaName, schema] of Object.entries(
      this.openAPISpec.components.schemas
    )) {
      if (schema.properties && schema.properties[fieldName]) {
        const fieldDef = schema.properties[fieldName];
        if (fieldDef && typeof fieldDef === "object" && fieldDef.description) {
