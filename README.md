...
**Auto-generated field detection is now fully metadata-driven:**
- If you have fields that are auto-generated by the backend (such as `id`, `createdAt`, `updatedAt`), add a `/// Auto-generated ...` comment above the field in your Prisma schema.
- These comments are preserved in the OpenAPI spec as `description` fields, and are detected by the test runner—even if your Zod schemas are regenerated by HeyAPI.
- The test runner will automatically omit these fields from request bodies for POST/PUT operations, ensuring your contract tests are always correct and future-proof.
- No pattern-based or hard-coded logic is used for auto-generated field detection—**only explicit metadata**.
...
> **Note:**
> The dynamic contract tester will automatically detect and omit auto-generated fields from test data if you annotate them in your Prisma schema with `/// Auto-generated ...` comments. This works even if your Zod schemas are regenerated by HeyAPI, as the metadata is preserved through the OpenAPI pipeline.
